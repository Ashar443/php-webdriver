...
class WebDriverExpectedCondition {
   ...
   * @return bool
   */
  public static function visible(WebDriverBy $by) {
    return new WebDriverExpectedCondition(
      function ($driver) use ($by) {
        try {
          $element = $driver->findElement($by);
  ...
  public static function invisible(WebDriverBy $by) {
    return new WebDriverExpectedCondition(
      function ($driver) use ($by) {
        try {
          $element = $driver->findElement($by);
   ...
   *
   * @param $selectors An array of WebDriverBy selectors and the type of thing to wait for (visible, invisible)
   * @return bool/object
   */
  public static function waitForMulti( $selectors = array() ) {
    return new WebDriverExpectedCondition(
      function ($driver) use ($selectors) {
        try {
	      foreach($selectors as $selector){
	        $element = $driver->findElement($selector['by']);
	        if(strtolower($selector['type']) == "visible"){
		        if($element->isDisplayed() === true){
					return $selector['by'];
		        }
	        }else if(strtolower($selector['type']) == "invisible"){
		        if($element->isDisplayed() !== true){
		          return $selector['by'];
		        }
	        }
